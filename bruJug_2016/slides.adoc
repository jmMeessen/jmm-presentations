...
image::images/SystemUnderTest.png[width=80%]
...
* thinks it is alone in the world (JVM defaults are picked up on the machine characteristics)
* watch out when several JVMs (in Docker) are running on the same machine
